...
import { Box, Button, Typography, IconButton, RadioGroup, FormControlLabel, Radio, LinearProgress } from "@mui/material";
            ...
            <>
                ...
                {videoFile && (
                        <>
                            ...
                            <video ref={videoRef} src={URL.createObjectURL(videoFile)} controls style={{ width: "100%", maxHeight: 300, borderRadius: 8 }} />
                            <IconButton onClick={() => { setVideoFile(null); setAutoThumbnailFile(null); }}>
                ...
                <Box>
                    ...
                    <RadioGroup row value={thumbnailMode} onChange={(e) => setThumbnailMode(e.target.value)}>
                        <FormControlLabel
                                value="auto"
                                control={<Radio sx={{ color: "#aaa", '&.Mui-checked': { color: "#42a5f5" } }} />}
                                label="자동 생성"
                                sx={{ color: "#fff" }}
                        />
                        <FormControlLabel
                ...
                {thumbnailMode === "auto" && autoThumbnailFile && (
                        <Box>
                            ...
                            <img src={URL.createObjectURL(autoThumbnailFile)} alt="auto" style={{ width: "100%", borderRadius: 8 }} />
                        </Box>
                )}
                ...
                {thumbnailMode === "custom" && (
                        <>
                            <Button variant="outlined" component="label" startIcon={<CloudUploadIcon />}>
                                썸네일 이미지 업로드
                                <input type="file" hidden accept="image/*" onChange={handleCustomThumbnailChange} />
                            </Button>
                            {customThumbnailFile && (
                                    <Box>
                                        <Typography color="gray">{customThumbnailFile.name}</Typography>
                                        <img src={URL.createObjectURL(customThumbnailFile)} alt="custom" style={{ width: "100%", borderRadius: 8 }} />
